...
dataChoosed1 <- dataChoosed[!apply(dataChoosed[, c(
  ...
  "Wahlbeteiltigung Europawahl 2024",
  "Wahlbeteiltigung Bekanntenkreis Bundestagswahl",
  "Print: Nutzung, politisch, Wochenzeitschriften")], 1, function(row) any(row %in% invalid_values)),]


View(dataChoosed1)

##############################Datenprüfung und -anpassung#######################
...
# Überprüfen der Ergebnisse
summary(WahlEU)
table(dataChoosed1$`politisches Interesse`)
table(`politisches Interesse`)
...
#(Intercept)  3.1238095         NA      NA       NA
#WahlEU_bin  -0.7754148         NA      NA       NA

plot(`politisches Interesse`,
     WahlEU,      ylab= "Gewählt Ja / Nein", 
     xlab = "politisches Interesse")
abline(0.17,0.018)
#Fazit: es können keine Rückschlüsse auf einen linearen Zusammenhang vollzogen werden
#logistische Regression kann durchgeführt werden
...
dataChoosed1_logreg <- glm(WahlEU_bin ~ `politisches Interesse`, family=binomial)
...
coefficients(summary(dataChoosed2_logreg))
#Coefficients:
#                             Estimate Std. Error z value Pr(>|z|)    
...
coefficients(summary(dataChoosed3_logreg))
#Coefficients:
#                                       Estimate Std. Error z value Pr(>|z|)    
...
coefficients(summary(dataChoosed4_logreg))
#Coefficients:
#                                                   Estimate Std. Error z value Pr(>|z|)    
...
dataChoosed5_logreg <- glm(WahlEU ~ Geschlecht , family=binomial)
#summary(dataChoosed6_logreg)
coefficients(summary(dataChoosed5_logreg))
#Coefficients:
#                     Estimate Std. Error z value Pr(>|z|)    
...
dataChoosed6_logreg <- glm(WahlEU ~ `Print: Nutzung, politisch, Wochenzeitschriften` , family=binomial)
#summary(dataChoosed7_logreg)
coefficients(summary(dataChoosed6_logreg))
#Coefficients:
#                                                   Estimate Std. Error z value Pr(>|z|)    
...
#Hypothese 5 und 6 weisen den geringsten Einfluss auf die Wahrscheinlichkeit zur Wahlbeteiltigung


###########weitere Berechnugen zu Hypothese 1: 
###########Wahrscheinlichkeit zur Wahlbeteiltigung von polit. Interesse abhängig
...
points(`politisches Interesse`, WahlEU_bin, pch=16)
...
### Nullmodel schaetzen
dataChoosed1_0 <- glm(WahlEU_bin ~ 1, family = binomial)
...
dataChoosed1_4 <- glm(WahlEU_bin ~ `politisches Interesse` + 
                        `Wahlbeteiltigung Bekanntenkreis Bundestagswahl` + 
                        + `demokratie Zufriedenheit`
                      + `Print: Nutzung, politisch, Wochenzeitschriften`,
                      family = binomial)
...
###
#
# Da der p-Wert bei Print sehr klein ist, kann man das Modell  um diese Spalte reduzieren
